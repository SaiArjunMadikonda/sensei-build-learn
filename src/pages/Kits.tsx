          ...
          <div className="grid gap-8 md:grid-cols-3">
            {kits.map((kit, i) => (
              <ScrollReveal key={kit.id} delay={i * 0.12}>
                <Card className="glass group flex h-full flex-col overflow-hidden rounded-3xl shadow-warm transition-all duration-300 hover:shadow-warm-lg hover:-translate-y-1 hover:border-primary/30">
...
const KitDetailPage = () => {
  ...
  return (
    <Layout>
      <section ref={heroRef} className="relative overflow-hidden py-12" style={{ background: 'linear-gradient(180deg, hsl(250 100% 98%) 0%, hsl(320 100% 99.6%) 100%)' }}>
        <motion.div style={{ y: blobY1 }} className="pointer-events-none absolute -right-32 -top-32 h-[500px] w-[500px] rounded-full opacity-[0.12] animate-mesh-float" aria-hidden="true">
          <div className="h-full w-full rounded-full" style={{ background: 'radial-gradient(circle, hsl(258 90% 76%) 0%, transparent 70%)', filter: 'blur(100px)' }} />
        </motion.div>
        <div className="container">
          ...
          <div className="grid gap-10 lg:grid-cols-2">
            ...
            <ScrollReveal direction="right" delay={0.15}>
              <div>
                ...
                <h1 className="font-display text-3xl tracking-tight">{kit.name}</h1>
      ...
      <section className="py-16">
        <div className="container">
          <div className="grid gap-12 lg:grid-cols-2">
            <ScrollReveal direction="left">
              <div>
                <h2 className="flex items-center gap-2 font-display text-2xl"><Package className="h-5 w-5 text-primary" /> What's Inside</h2>
            ...
            <ScrollReveal direction="right">
              <div>
                <h2 className="font-display text-2xl">What You'll Learn</h2>
                <ul className="mt-4 space-y-2">
      ...
      <section className="bg-accent py-12">
        <div className="container">
          <ScrollReveal>
            <div className="glass flex items-start gap-3 rounded-3xl border p-6 shadow-warm">
          ...
          <Accordion type="single" collapsible className="mt-6 max-w-2xl space-y-2">
            {kit.faqs.map((faq, i) => (
              <ScrollReveal key={i} delay={i * 0.06}>
                <AccordionItem value={`item-${i}`} className="glass rounded-2xl border px-5">
