          ...
          <div className="mx-auto mt-14 max-w-2xl space-y-4">
            {steps.map((step, i) => (
              <ScrollReveal key={step.num} delay={i * 0.08}>
                <div className="flex items-center gap-5 rounded-3xl border border-border bg-card p-5 shadow-warm transition-all duration-300 hover:shadow-warm-md hover:-translate-y-0.5 hover:border-primary/30">
            ...
            <Accordion type="single" collapsible className="mt-10 space-y-2">
              {minyFaqs.map((faq, i) => (
                <ScrollReveal key={i} delay={i * 0.06}>
                  <AccordionItem value={`miny-faq-${i}`} className="rounded-2xl border border-border bg-card px-5 shadow-warm transition-all duration-200 hover:shadow-warm-md">
